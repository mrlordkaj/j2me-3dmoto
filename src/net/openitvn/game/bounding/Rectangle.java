...
public class Rectangle extends Shape {
    ...
    private void updateVertex() {
        ...
        Point absPivot = new Point(x + pivot.x, y + pivot.y);
        switch (angle) {
            case 0:
            ...
            case 90:
                vertex[3] = new Point(absPivot.x + pivot.y, absPivot.y + (width - pivot.x));
                vertex[0] = new Point(vertex[3].x, absPivot.y - pivot.x);
                vertex[1] = new Point(absPivot.x - (height - pivot.y), vertex[0].y);
                vertex[2] = new Point(vertex[1].x, vertex[3].y);
                break;
                
            case 180:
                vertex[2] = new Point(absPivot.x - (width - pivot.x), absPivot.y + pivot.y);
                vertex[3] = new Point(absPivot.x + pivot.x, vertex[2].y);
                vertex[0] = new Point(vertex[3].x, (y + pivot.y) - (height - pivot.y));
                vertex[1] = new Point(vertex[2].x, vertex[0].y);
                break;
                
            case 270:
                vertex[1] = new Point(absPivot.x - pivot.y, absPivot.y - (width - pivot.x));
                vertex[2] = new Point(vertex[1].x, absPivot.y + pivot.x);
                vertex[3] = new Point(absPivot.x + (height - pivot.y), vertex[2].y);
            ...
            default:
                ...
                vertex[2] = new Point(x, y + height);
                vertex[3] = new Point(x + width, y + height);
                rotatePoint(vertex[0], absPivot, radAngle);
                rotatePoint(vertex[1], absPivot, radAngle);
                rotatePoint(vertex[2], absPivot, radAngle);
                rotatePoint(vertex[3], absPivot, radAngle);
                break;
        }
    ...
    public boolean contains(Point p) {
        ...
        switch (angle) {
            ...
            default:
                if (new Triangle(vertex[0], vertex[1], vertex[2]).contains(p))
                    return true;
                if (new Triangle(vertex[0], vertex[3], vertex[2]).contains(p))
                    return true;
                return false;
        }
    }
    ...
    public boolean collides(Triangle target) {
        ...
        for (byte i = 0; i < target.vertex.length; i++) {
            if (contains(target.vertex[i]))
                return true;
        }
        for (byte i = 0; i < vertex.length; i++) {
            if (target.contains(vertex[i]))
                return true;
        }
        return false;
    }
    ...
    public boolean collides(Rectangle target) {
        ...
        if ((target.angle == 0 || target.angle == 90 || target.angle == 180 || target.angle == 270)
            ...
            if (Math.max(vertex[0].y, target.vertex[0].y) >= Math.min(vertex[2].y, target.vertex[2].y)) 
                return false;
