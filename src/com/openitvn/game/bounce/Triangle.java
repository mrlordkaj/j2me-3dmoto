...
public class Triangle extends Shape {
    ...
    public boolean contains(Point p) {
        Point e0 = Point.minus(p, vertex[0]);
        Point e1 = Point.minus(vertex[1], vertex[0]);
        Point e2 = Point.minus(vertex[2], vertex[0]);
    ...
    public boolean collide(Triangle target) {
        if(contains(target.vertex[0])) return true;
        if(contains(target.vertex[1])) return true;
        if(contains(target.vertex[2])) return true;
        if(target.contains(vertex[0])) return true;
        if(target.contains(vertex[1])) return true;
